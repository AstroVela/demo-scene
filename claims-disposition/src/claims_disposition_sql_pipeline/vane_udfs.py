...
_NO_DAMAGE_MARKERS = {"", "unknown", "none", "none_visible", "no_damage"}
...
@vane.func(return_dtype="BOOLEAN", name="json_list_has_meaningful_damage")
def json_list_has_meaningful_damage(value: str | None) -> bool:
    try:
    ...
    if not isinstance(parsed, list):
        return False
    return any(str(item).strip().lower() not in _NO_DAMAGE_MARKERS for item in parsed)
...
CLAIM_MATERIAL_BATCH_SCHEMA = {
    ...
    "bucket": "VARCHAR",
    "object_key": "VARCHAR",
    "supported_role_media": "BOOLEAN",
    "duplicate_file_id": "BOOLEAN",
    "duplicate_file_order": "BOOLEAN",
    "material_locator_valid": "BOOLEAN",
    "runtime_locator_valid": "BOOLEAN",
    "object_exists": "BOOLEAN",
    "object_sha256": "VARCHAR",
    "photo_quality_json": "VARCHAR",
    "document_ocr_json": "VARCHAR",
    "document_fields_json": "VARCHAR",
    "document_quality_json": "VARCHAR",
    "photo_usable": "BOOLEAN",
    "photo_quality_score": "DOUBLE",
    "document_usable": "BOOLEAN",
}
_CLAIM_MATERIAL_ARROW_SCHEMA = pa.schema(
    [
        ("claim_id", pa.string()),
        ("scenario", pa.string()),
        ("description", pa.string()),
        ("submitted_at", pa.timestamp("us", tz="UTC")),
        ("is_test_claim", pa.bool_()),
        ("material_index", pa.int64()),
        ("file_id", pa.string()),
        ("file_order", pa.int32()),
        ("role", pa.string()),
        ...
        ("supported_role_media", pa.bool_()),
        ("duplicate_file_id", pa.bool_()),
        ("duplicate_file_order", pa.bool_()),
        ("material_locator_valid", pa.bool_()),
        ("runtime_locator_valid", pa.bool_()),
        ("object_exists", pa.bool_()),
        ("object_sha256", pa.string()),
        ("photo_quality_json", pa.string()),
        ("document_ocr_json", pa.string()),
        ("document_fields_json", pa.string()),
        ("document_quality_json", pa.string()),
        ("photo_usable", pa.bool_()),
        ("photo_quality_score", pa.float64()),
        ("document_usable", pa.bool_()),
    ]
)


def build_claim_material_batch_actor(
    minio_config: MinioConfig,
    *,
    required_fields: tuple[str, ...],
    minimum_text_confidence: float,
) -> type:
    """Build a batch actor that enriches material rows through the active Runner."""

    row_actor_class = DocumentOcrActor.user_class

    class ClaimMaterialBatchActor:
        def __init__(self) -> None:
            self.store = MinioStore(minio_config)
            self.ocr_actor = row_actor_class(minio_config)
        ...
        def __call__(self, batch: Any) -> pa.RecordBatch:
            ...
            for source in batch.to_pylist():
                ...
                runtime_locator_valid = bool(
                    ...
                    and source["bucket"] == minio_config.bucket
                )
                object_exists = bool(
                    runtime_locator_valid
                    and self.store.exists(source["bucket"], source["object_key"])
                )
                object_sha256 = (
                    self.store.sha256(source["bucket"], source["object_key"])
                    if object_exists
                    else None
                )
                photo_quality_json = None
                document_ocr_json = None
                document_fields_json = None
                document_quality_json = None
                photo_usable = False
                ...
                if (
                    ...
                    photo_quality = analyze_photo_bytes(value)
                    photo_quality_json = stable_json(photo_quality)
                    photo_usable = bool(photo_quality.get("photo_usable", False))
                    photo_quality_score = float(
                        photo_quality.get("quality_score", 0.0)
                    )
                ...
                if (
                    ...
                    ocr = json.loads(document_ocr_json)
                    fields = extract_document_fields(ocr)
                    document_fields_json = stable_json(fields)
                    ...
                    document_quality_json = stable_json(quality)
                    document_usable = bool(quality["document_usable"])
                ...
                rows.append(
                    {
                        **{
                            ...
                            for name in (
                                ...
                                "material_locator_valid",
                            )
                        },
                        "runtime_locator_valid": runtime_locator_valid,
                        "object_exists": object_exists,
                        "object_sha256": object_sha256,
                        "photo_quality_json": photo_quality_json,
                        "document_ocr_json": document_ocr_json,
                        "document_fields_json": document_fields_json,
                        "document_quality_json": document_quality_json,
                        "photo_usable": photo_usable,
            ...
            return pa.RecordBatch.from_pylist(
                rows,
                schema=_CLAIM_MATERIAL_ARROW_SCHEMA,
            )

    return ClaimMaterialBatchActor
...
DAMAGE_VALIDATION_BATCH_SCHEMA = {
    ...
    "photo_quality_json": "VARCHAR",
    "raw_damage_response": "VARCHAR",
    "damage_result_json": "VARCHAR",
    "model_status": "VARCHAR",
    "vehicle_visible": "BOOLEAN",
    "target_vehicle_clear": "BOOLEAN",
    "damage_visible": "BOOLEAN",
    "damaged_parts_json": "VARCHAR",
    "damage_types_json": "VARCHAR",
    "severity_hint": "VARCHAR",
    "uncertainty_reasons_json": "VARCHAR",
    "finding_determinate": "BOOLEAN",
    "damage_confidence": "DOUBLE",
    "meaningful_damaged_parts": "BOOLEAN",
    "meaningful_damage_types": "BOOLEAN",
    "uncertainty_reason_count": "INTEGER",
    "blocking_uncertainty_reason_count": "INTEGER",
    "positive_damage_result": "BOOLEAN",
    "negative_damage_result": "BOOLEAN",
}
_DAMAGE_VALIDATION_ARROW_SCHEMA = pa.schema(
    [
        ("claim_id", pa.string()),
        ("file_id", pa.string()),
        ("file_order", pa.int32()),
        ("photo_sha256", pa.string()),
        ("photo_quality_json", pa.string()),
        ("raw_damage_response", pa.string()),
        ("damage_result_json", pa.string()),
        ...
        ("finding_determinate", pa.bool_()),
        ("damage_confidence", pa.float64()),
        ("meaningful_damaged_parts", pa.bool_()),
        ("meaningful_damage_types", pa.bool_()),
        ("uncertainty_reason_count", pa.int32()),
        ("blocking_uncertainty_reason_count", pa.int32()),
        ("positive_damage_result", pa.bool_()),
        ("negative_damage_result", pa.bool_()),
    ]
)


def build_damage_validation_batch() -> Any:
    """Build the stateless batch callable used by the Runner damage stage."""

    validate_response = photo_damage_result_json.python_function
    has_meaningful_damage = json_list_has_meaningful_damage.python_function
    ...
    def validate_batch(batch: Any) -> pa.RecordBatch:
        ...
        for source in batch.to_pylist():
            ...
            payload = json.loads(damage_result_json)
            damaged_parts_json = stable_json(payload.get("damaged_parts", []))
            damage_types_json = stable_json(payload.get("damage_types", []))
            uncertainty_reasons = payload.get("uncertainty_reasons", [])
            if not isinstance(uncertainty_reasons, list):
                uncertainty_reasons = []
            uncertainty_reasons_json = stable_json(uncertainty_reasons)
            uncertainty_reason_count = len(uncertainty_reasons)
            finding_determinate = payload.get("finding_determinate")
            damage_confidence = payload.get("confidence")
            meaningful_damaged_parts = has_meaningful_damage(
                damaged_parts_json
            )
            meaningful_damage_types = has_meaningful_damage(
                damage_types_json
            )
            blocking_uncertainty_reason_count = (
                0 if finding_determinate is True else uncertainty_reason_count
            )
            model_status = payload.get("status")
            vehicle_visible = payload.get("vehicle_visible")
            ...
            successful = (
                ...
                and isinstance(damage_confidence, (int, float))
                and not isinstance(damage_confidence, bool)
                and float(damage_confidence) >= 0.80
                and vehicle_visible is True
                and target_vehicle_clear is True
                and blocking_uncertainty_reason_count == 0
            )
            positive_damage_result = bool(
                successful
                and damage_visible is True
                and meaningful_damaged_parts
                and meaningful_damage_types
                and severity_hint in {"minor", "moderate"}
            )
            negative_damage_result = bool(
                successful
                and damage_visible is False
                and not meaningful_damaged_parts
                and not meaningful_damage_types
                and severity_hint in {"none", "unknown"}
            )
            rows.append(
                {
                    **source,
                    "damage_result_json": damage_result_json,
                    ...
                    "uncertainty_reasons_json": uncertainty_reasons_json,
                    "finding_determinate": finding_determinate,
                    "damage_confidence": damage_confidence,
                    "meaningful_damaged_parts": meaningful_damaged_parts,
                    "meaningful_damage_types": meaningful_damage_types,
                    "uncertainty_reason_count": uncertainty_reason_count,
                    "blocking_uncertainty_reason_count": (
                        blocking_uncertainty_reason_count
                    ),
                    "positive_damage_result": positive_damage_result,
                    "negative_damage_result": negative_damage_result,
                }
            )
        return pa.RecordBatch.from_pylist(
            rows,
            schema=_DAMAGE_VALIDATION_ARROW_SCHEMA,
        )
