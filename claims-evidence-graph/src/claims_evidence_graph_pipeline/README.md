...
- `pipeline.py`: ingestion, Vane relation execution, SQL aggregation, and
  output materialization.
- `udfs.py`: photo quality and FUNSD document batch processors.
- `photo_vlm.py`: semantic photo evidence generation through
  `vane.ai.prompt(image_columns=["file_bytes"])`.
- `validation.py`: input and output validation gates.
- `qwen_openai_server.py`: optional local OpenAI-compatible Qwen adapter.

For public demos, start with the repository-level `README.md`. The default
public path is the synthetic fixture runner, which does not require raw claim
data, model weights, or external services.
